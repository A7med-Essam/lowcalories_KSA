...
export class SharedService {
  ...
  checkAllWeek(e: HTMLElement, ProgramDetailsForm?: any) {
    let AllSiblings: HTMLElement[] = this.getAllSiblings(
      e.parentElement,
  ...
  checkDays(
    ...
    if (
      ...
      siblings[0].children[0].classList.contains('active') ||
      siblings[1].children[0].classList.contains('active') ||
      siblings[2].children[0].classList.contains('active') ||
      siblings[3].children[0].classList.contains('active') ||
      siblings[4].children[0].classList.contains('active') ||
  ...
  onSelectedDate(SelectedDate: Date, deliveredDays: HTMLElement) {
    ...
    for (let i = 0; i < Days.length; i++) {
      ...
      if (
        Number(deliveredDays.children[i].children[0].getAttribute('day')) ==
        SelectedDate.getDay()
      ) {
  ...
  getKsaTime() {
    ...
    this.ksaDate = new Date(utc + 3600000 * 4); // 4 is the offset of KSA
    this.ksaDate.setDate(this.ksaDate.getDate() + 2);
    return this.ksaDate;
  }
